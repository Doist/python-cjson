...
encode_dict(PyObject *dict)
{
    ...
    while (PyDict_Next((PyObject *)mp, &i, &key, &value)) {
        ...
        if (!PyString_Check(key) && !PyUnicode_Check(key) && !PyInt_Check(key) && !PyLong_Check(key)) {
        ...
        if(PyInt_Check(key) || PyLong_Check(key)) {
            s = PyString_FromString("\"");
            PyString_ConcatAndDel(&s, encode_object(key));
...
static PyObject*
encode_date(PyObject *object) {
    PyObject *result, *temp;

    result = PyString_FromString("\"DATE[[");

    temp = PyObject_Str(object);
...
static PyObject*
encode_json_object(PyObject *object) {
    PyObject *result = PyObject_CallMethod(object, "__json__", NULL);
    PyObject *encoded_result = NULL;
    if (result == NULL) {
        return NULL;
    } else {
...
encode_object(PyObject *object)
{
    if (object == Py_True) {
        ...
        return PyObject_Str(object);
    // shortcuts: check if instance is exactly of a base type
    // (mind "CheckExact" here)
    ...
    // then check whether the object has __json__attribute
    } else if (PyObject_HasAttrString(object, "__json__")) {
        return encode_json_object(object);
    // then check whether instances are of the subclasses of base types
        ...
        return encode_dict(object);
    // last resort: it's a datetime
    } else if (object && PyObject_HasAttrString(object, "isoformat")) {
        return encode_date(object);
    } else {
